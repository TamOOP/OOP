...
public class IngredientDAOImpl implements IngredientDAO {
	...
	public Ingredient get(int idIngredient) throws SQLException {
		...
		if(rs.next()) {
			ingredient = new Ingredient(rs.getInt("IngrID"), rs.getString("IngrName"), rs.getInt("HasAmount"));
		}
		rs.close();
		ps.close();
		con.close();
	...
	public List<Ingredient> getAll() throws SQLException {
		...
		if(rs != null) {
			...
			while(rs.next()) {
				ingredients.add(new Ingredient(rs.getInt("IngrID"), rs.getString("IngrName"), rs.getInt("HasAmount")));
			}
		}
		rs.close();
		ps.close();
	...
	public int insert(Ingredient ingredient) throws SQLException {
		Connection con = FunctionsDatabase.getConnection();
		String sql = "INSERT INTO Tb2_Ingredient (IngrName, HasAmount) VALUES (?, ?)";
		PreparedStatement ps = con.prepareStatement(sql);
	...
	public int update(Ingredient ingredient) throws SQLException {
		Connection con = FunctionsDatabase.getConnection();
		String sql = "UPDATE Tb2_Ingredient SET HasAmount = ? WHERE IngrName = ?";
		PreparedStatement ps = con.prepareStatement(sql);
		ps.setInt(1, ingredient.getHasQuantity());
	...
	public List<Ingredient> getIngredientList(int idFood) throws SQLException {
		List<Ingredient> ingredients  = null;
		Connection con = FunctionsDatabase.getConnection();
		String sql = "SELECT IngrID, IngrName, HasAmount FROM GetFoodIngredient WHERE FoodID = ?";
		PreparedStatement ps = con.prepareStatement(sql);
		ps.setInt(1, idFood);
		ResultSet rs = ps.executeQuery();
		if(rs != null) {
	...
	public Ingredient getIngredient(String name) throws SQLException {
        ...
        public Ingredient getIngr(String name) throws SQLException{
            ...
            if(rs.next()){
                ...
                Ingr = new Ingredient(ID,iname,bool);
            }
            rs.close();
            ps.close();
            con.close();
