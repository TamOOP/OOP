...
public class FoodAndIngredientDAOImpl implements FoodAndIngredientDAO {
	...
	public FoodAndIngredient get(int id) throws SQLException {
        ...
        public List<FoodAndIngredient> getFis(int idFood) throws SQLException {
		List<FoodAndIngredient> fais = null;
		Connection con = FunctionsDatabase.getConnection();
		String sql = "SELECT * FROM R1_Food_Ingredient WHERE FoodID = ?";
		...
		if (rs != null) {
			...
			while (rs.next()) {
                            ...
                            fais.add(new FoodAndIngredient(rs.getInt("FoodID"), 
                                    rs.getInt("IngrID"), amount, 
                                    rs.getString("Unit")));
                        }
		}
		rs.close();
