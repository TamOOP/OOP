...
public class Menu_FoodDAOimpl implements Menu_FoodDAO {
	...
	public List<Menu_Food> getAll() throws SQLException {
		...
		if(rs != null) {
			...
			while(rs.next()) {
				...
				Menu_Food f = new Menu_Food(i1, i2, i3);
				mf.add(f);
			}
		}
		rs.close();
        ...
        public List<Menu_Food> getAll(int mid, String tid) throws SQLException{
            Connection c = FunctionsDatabase.getConnection();
            List<Menu_Food> mf = null;
            String sql = "SELECT * FROM R2_Menu_Food WHERE Mid=? AND time= ?";
            PreparedStatement ps = c.prepareStatement(sql);
            ps.setInt(1, mid);
            ps.setString(2, tid);
            ResultSet rs = ps.executeQuery();
	...
	public List<Menu_Food> getAll(int mid) throws SQLException{
            Connection c = FunctionsDatabase.getConnection();
            List<Menu_Food> mf = null;
