...
public class StepDAOImpl implements StepDAO {
	...
	public Step get(int id) throws SQLException {
		...
		Connection con = FunctionsDatabase.getConnection();
                String sql = "SELECT * FROM Tb3_Step WHERE StepIndex=?";
                PreparedStatement ps = con.prepareStatement(sql);
                ps.setInt(1, id);
                ResultSet rs = ps.executeQuery();
                if(rs.next()){
                    step = new Step(rs.getInt("FoodID"), rs.getInt("StepIndex"), rs.getString("StepContent"), rs.getString("StepImage"));
                }
                rs.close();
		ps.close();
		con.close();
	...
	public int save(Step step) throws SQLException {
		...
		if(rs != null) {
			...
			while(rs.next()) {
				...
				idSteps.add(rs.getInt("StepIndex"));
			}
		}
		rs.close();
		ps.close();
		...
		int check = 0;
                for(int i = 0 ; i < idFoods.size(); i ++){
                    if (idFoods.get(i).equals(step.getIdFood()) && idSteps.get(i).equals(step.getIdStep())){
                    check = 1;
                    }
                }
		if(check == 1){
